              ...
              <form onSubmit={handleProfileSubmit} className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="number" required value={profile.age} onChange={e => setProfile({...profile, age: parseInt(e.target.value)})} className="w-full bg-slate-50 border-none rounded-2xl p-5 text-lg outline-none focus:ring-2 focus:ring-[#064e3b]/10"/>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">District / ضلع</label>
                    <select value={profile.district} onChange={e => setProfile({...profile, district: e.target.value})} className="w-full bg-slate-50 border-none rounded-2xl p-5 text-lg outline-none focus:ring-2 focus:ring-[#064e3b]/10">
                  ...
                  <div className="flex gap-2">
                    {['Single', 'Married', 'Divorced', 'Widow'].map(s => (
                      <button 
                        ...
                        className={`flex-1 py-4 rounded-2xl text-[10px] font-bold transition-all ${profile.maritalStatus === s ? 'bg-[#064e3b] text-white shadow-lg scale-[1.05]' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}
                      >
                        {s}
                      </button>
                    ))}
                ...
                <div className="bg-white p-12 rounded-[4rem] shadow-sm border border-slate-50">
                  ...
                  <div className="relative">
                    <textarea 
                      ...
                      className={`w-full bg-slate-50 border-none rounded-[2rem] p-8 min-h-[180px] text-lg focus:ring-2 focus:ring-[#064e3b]/10 transition-all outline-none placeholder:text-slate-300 ${isUrdu ? 'text-right' : ''}`}
                      onKeyDown={(e) => {
